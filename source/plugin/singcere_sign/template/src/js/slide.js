...
Weather.prototype = {
  getWeather: function(city, ext, showid) {
    this.city = city;
    this.ext = ext;
    this.showid = showid;
    var thisTemp = this;
    jq163.getScript("http://php.weather.sina.com.cn/iframe/index/w_cl.php?code=js&day=1&city=" + city + "&dfc=3", function() { thisTemp.echo(); });
  },
  proc_wimg: function(weather) {
    ...
    if (weather.indexOf("多云") !== -1 || weather.indexOf("晴") !== -1) {
        ...
        return '<img src="' + style_img + '" width="48" height="48"' + 'title="' + window.SWther.w[this.city][0].s1 + '">';
  },
  echo: function() {
    var today = new Date();
        ...
        weekday[5] = "星期五";
        weekday[6] = "星期六";
        jq163('#' + this.showid).html('<span class="wtimg" id="T_weather_img">' + this.proc_wimg(window.SWther.w[this.city][0].s1) + '</span>' + this.city + '<span id="T_weather">' + window.SWther.w[this.city][0].s1 + '</span>' + '<span id="T_temperature">' + window.SWther.w[this.city][0].t1 + '&deg;C~' +  window.SWther.w[this.city][0].t2 + '&deg;C' + '</span>' + this.ext);
    }
}
