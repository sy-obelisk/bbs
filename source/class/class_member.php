	...
	function on_register() {
		...
		if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
			...
			if(!$activation) {
                ...
                } elseif(!preg_match("/^[a-zA-Z\d]\w{1,13}[a-zA-Z\d]$/", $username)){
                    showmessage('profile_username_china');
                }
			...
			if(!$activation) {
				$uid = uc_user_register(addslashes($username), $password, $email,'','论坛','', $questionid, $answer, $_G['clientip']);
				if($uid <= 0) {
					if($uid == -1) {
